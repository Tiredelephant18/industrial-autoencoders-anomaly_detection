...
class C3DAutoencoder(nn.Module):
    ...
    def __init__(
        ...
        self._enc_shape, self.flatten_dim = self._compute_enc_shape()
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        x = self.fc_decode(z).view(-1, *self._enc_shape[1:])
    ...
    def describe(self):
        ...
        print(f"  enc_shape={list(self._enc_shape)}")
        print(f"  flatten_dim={self.flatten_dim}  latent_dim={self.latent_dim}")
        print(f"{'─'*60}")
        ...
        with torch.no_grad():
            ...
            x2 = self.fc_encode(x2.view(1, -1))
            x2 = self.fc_decode(x2).view(self._enc_shape)
